...
Point road_with_track::agv_track_detection(Mat& img)
{
	...
	*/
	...
	for (size_t i = 0; i < lines.size(); i++)
	{
		...
		if (angle > 20 && angle < 160) {
			...
			line(img, Point(x1, y1), Point(x2, y2), Scalar(0, 0, 255), 3, CV_AA);
		}
	}
	...
	if (cnt_l && cnt_r) {
		...
		double x_l = (img.rows * 0.65 - b_l) / k_l;
		double x_r = (img.rows * 0.65 - b_r) / k_r;
		target = Point((x_l + x_r) / 2, img.rows * 0.65);
	}
	Point origin(img.cols * 0.51, img.rows - 1);
	line(img, origin, target, Scalar(255, 255, 0), 3);
...
double road_with_track::agv_navigate(const Mat& img)
{
	Mat img_copy; img.copyTo(img_copy);
	const Point p = agv_track_detection(img_copy);
	double k = (p.x - img_copy.cols * 0.51) / f_dx;
